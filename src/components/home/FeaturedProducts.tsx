...
const productRing = getMediaUrl("product-ring", "image");
const productEarrings = getMediaUrl("product-earrings", "image");
const productBracelet = getMediaUrl("product-bracelet", "image");
    ...
    <section className="py-24 bg-background">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <p className="font-body text-xs tracking-[0.3em] uppercase text-muted-foreground mb-4">Curated for You</p>
            ...
            <div key={product.id} className="group cursor-pointer" style={{ animationDelay: `${index * 100}ms` }}>
              <Link to="/shop">
                <div className="relative aspect-[3/4] mb-4 overflow-hidden bg-muted">
                  ...
                  {product.isNew && (
                    <span className="absolute top-4 left-4 bg-primary text-primary-foreground text-[10px] tracking-widest uppercase px-3 py-1">New</span>
                  )}
                  ...
                  <button onClick={(e) => handleWishlist(e, product.id)}
                    ...
                    <Heart className={`w-4 h-4 ${isWishlisted(product.id) ? "fill-red-500 text-red-500" : "text-foreground"}`} />
                  </button>
                </div>
                <div className="text-center">
                  <h3 className="font-display text-lg text-foreground mb-1 group-hover:text-primary transition-colors">{product.name}</h3>
                  <p className="font-body text-sm text-muted-foreground">{formatPrice(product.price)}</p>
                </div>
              </Link>
              <Button variant="outline" size="sm" className="w-full text-xs h-8 rounded-xl mt-2">Add to Cart</Button>
            </div>
          ))}
        </div>
