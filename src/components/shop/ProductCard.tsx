...
const ProductCard = ({ product, collectionBadge }: ProductCardProps) => {
  ...
  const handleWishlist = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    toggle(productIdStr);
    toast.success(isWishlisted(productIdStr) ? "Removed from wishlist" : "Added to wishlist");
  };
    ...
    <div className="group">
      <Link to={`/product/${product.slug}`}>
        ...
        >
          ...
          >
            <Heart className={`w-4 h-4 ${isWishlisted(productIdStr) ? "fill-red-500 text-red-500" : "text-foreground"}`} />
          </button>
        </div>

        <div className="space-y-1">
          <h3 className="font-display text-sm text-foreground line-clamp-1">{product.name}</h3>
          <div className="flex items-center gap-2">
            {hasDiscount && (
              <span className="font-body text-xs text-muted-foreground line-through">{formatPrice(product.comparePrice)}</span>
            )}
            <span className="font-body text-sm font-medium text-foreground">{formatPrice(product.price)}</span>
          </div>
      ...
      <Button variant="outline" size="sm" className="w-full text-xs h-8 rounded-xl mt-2" onClick={handleAddToBag}>
        Add to Cart
      </Button>
    </div>
  );
