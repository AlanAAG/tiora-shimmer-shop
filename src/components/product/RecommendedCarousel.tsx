                ...
                <Link to={`/product/${product.slug}`} className="group block">
                  <div className="relative aspect-[3/4] mb-3 overflow-hidden bg-muted rounded-2xl border border-border">
                    {product.isBestSeller && (
                      <span className="absolute top-2 left-2 z-10 bg-foreground/90 text-background text-[10px] tracking-widest uppercase px-2 py-1 rounded-lg font-body">Best Seller</span>
                    )}
                    <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    <button onClick={(e) => handleWishlist(e, String(product.id))}
                      ...
                      <Heart className={`w-4 h-4 ${isWishlisted(String(product.id)) ? "fill-red-500 text-red-500" : "text-foreground"}`} />
                    </button>
                  </div>
                  ...
                  <Button variant="outline" size="sm" className="w-full text-xs h-8 rounded-xl">Add to Cart</Button>
                </Link>
              </CarouselItem>
            ))}
          </CarouselContent>
